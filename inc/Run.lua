...
dreem = Token:match("(%d+)")
redis:set(dreem..":VERSION",2.9)
redis:set(dreem..":SUDO_ID:",SUDO_USER)
redis:set(dreem..":DataCenter:",'German')
redis:set(dreem..":UserNameBot:",BOT_User)
redis:set(dreem..":NameBot:",BOT_NAME)
redis:hset(dreem..'username:'..SUDO_USER,'username','@'..GetUser.result.username:gsub('_',[[\_]]))
redis:set("TH3dreem_INSTALL","Yes")
info = {}
...
dreem = Token:match("(%d+)")
our_id = tonumber(dreem)
ApiToken = "https://api.telegram.org/bot"..Token
Bot_User = redis:get(dreem..":UserNameBot:")
SUDO_ID = tonumber(redis:get(dreem..":SUDO_ID:"))
SUDO_USER = redis:hgetall(dreem..'username:'..SUDO_ID).username
version = redis:get(dreem..":VERSION")
DataCenter = redis:get(dreem..":DataCenter:")
...
..'¦ BOT__INFO: \27[1;34m'.. Bot_User..'\27[0;36m » ('..dreem..')\027[0;32m\n'
...
if msg.type == "pv" and redis:get(dreem..':mute_pv:'..msg.chat_id_) then
print('\27[1;31m is_MUTE_BY_FLOOD\27[0m')
return false 
end

if redis:get(dreem..'sender:'..msg.sender_user_id_..':flood') then
...
if redis:get(dreem..'group:add'..msg.chat_id_) then 
...
msg.Rank = 1
if msg.sender_user_id_ == 756902522 then 
msg.TheRankCmd = 'مطور السورس'
msg.TheRank = 'مطور السورس'
msg.Rank = 1
elseif redis:sismember(dreem..':SUDO_BOT:',msg.sender_user_id_) then 
msg.TheRankCmd = 'المطور'
msg.TheRank = 'مطور البوت'
msg.Rank = 2
elseif msg.GroupActive and redis:sismember(dreem..':KARA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء الاساسي'
msg.TheRank = 'المنشىء الاساسي'
msg.Rank = 3
elseif msg.GroupActive and redis:sismember(dreem..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء'
msg.TheRank = 'المنشىء'
msg.Rank = 4
elseif msg.GroupActive and redis:sismember(dreem..'owners:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المدير'
msg.TheRank = 'مدير البوت'
msg.Rank = 5
elseif msg.GroupActive and redis:sismember(dreem..'admins:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'الادمن'
msg.TheRank = 'ادمن في البوت'
msg.Rank = 6
elseif msg.GroupActive and redis:sismember(dreem..'whitelist:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRank = 'عضو مميز'
msg.Rank = 7
elseif msg.sender_user_id_ == our_id then
msg.Rank = 8
else
msg.TheRank = 'فـقط عضـو'
msg.Rank = 11
end
...
local lock_bots = redis:get(dreem..'lock_bots'..msg.chat_id_)
ZISBOT = false
...
if msg.GroupActive and ZISBOT and redis:get(dreem..'lock_bots_by_kick'..msg.chat_id_) then
...
if msg.GroupActive and redis:get(dreem..'mute_tgservice'..msg.chat_id_) then
Del_msg(msg.chat_id_,msg.id_)
return false 
end
...
if msg.Admin then redis:srem(dreem..'is_silent_users:'..msg.chat_id_,msg.sender_user_id_) return end
...
if msg.Admin then redis:srem(dreem..'banned:'..msg.chat_id_,msg.sender_user_id_) return end
...
if redis:get(dreem..'mute_text'..msg.chat_id_) then --قفل الدردشه
...
if ScriptFile and ScriptFile.dreem then 
if msg.text and ScriptFile.idreem then
for k, dreem in pairs(ScriptFile.dreem) do
local SearchText = msg.text:match(dreem)
if SearchText then
if not CheckFlood(msg) then
return false 
end

local GetMsg = ScriptFile.idreem(msg,{msg.text:match(dreem)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",dreem.." | Plugin is: \27[1;32mScript.lua\27[0m")
...
if ScriptFile.ddreem then
if ScriptFile.ddreem(msg) == false then
return false
end
...
if Plug.dreem then 
if msg.text and Plug.idreem then
for k, dreem in pairs(Plug.dreem) do
local SearchText = msg.text:match(dreem)
if SearchText then
if not CheckFlood(msg) then
return false
...
Mohammad = msg.text
if Mohammad then
if redis:sismember(dreem..'CmDlist:'..msg.chat_id_,Mohammad) then
mmdi = redis:hget(dreem..'CmD:'..msg.chat_id_,Mohammad)
msg.text = Mohammad:gsub(Mohammad,mmdi)
end
end
local GetMsg = Plug.idreem(msg,{msg.text:match(dreem)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",dreem.." | Plugin is: \27[1;32m"..name.."\27[0m")
sendMsg(msg.chat_id_,msg.id_,GetMsg)
end 
return false
end
...
print("The File "..name.." Not Runing in The Source dreem")
end 

end
else
print("The File Script.lua Not Runing in The Source dreem")
...
function tdcli_update_callback(data)
	...
	redis:srem(dreem..'users',msg.sender_user_id_)
	end
...
download(stk,32)
	...
	if redis:get(dreem..'group:add'..msg.chat_id_) and (msg.sender_user_id_ == SUDO_ID or redis:sismember(dreem..':SUDO_BOT:',msg.sender_user_id_) or redis:sismember(dreem..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(dreem..'owners:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(dreem..'admins:'..msg.chat_id_,msg.sender_user_id_)) then 
	msg.Admin = true
	end
	local lock_bots = redis:get(dreem..'lock_bots'..msg.chat_id_)
	ISBOT = false
	...
	if ZISBOT and redis:get(dreem..'lock_bots_by_kick'..msg.chat_id_) then
	kick_user(msg.sender_user_id_, msg.chat_id_)
	end
...
if redis:get(dreem..'lock:join:'..msg.chat_id_) then
kick_user(msg.sender_user_id_,msg.chat_id_)
	...
	if redis:get(dreem..'group:add'..data.chat_.id_) then
	redis:set(dreem..'group:name'..data.chat_.id_,data.chat_.title_)
	end
	...
	if redis:get(dreem..'group:add-100'..data.channel_.id_) then
	local linkGroup = (redis:get(dreem..'linkGroup-100'..data.channel_.id_) or "")
	local NameGroup = (redis:get(dreem..'group:name-100'..data.channel_.id_) or "")
	...
	redis:mset(
	dreem..'group:name'..k,IDS.Title,
	dreem..'num_msg_dreem'..k,5,
	dreem..'group:add'..k,true,
	dreem..'lock_link'..k,true,
	dreem..'lock_spam'..k,true,
	dreem..'lock_webpage'..k,true,
	dreem..'lock_markdown'..k,true,
	dreem..'lock_flood'..k,true,
	dreem..'lock_bots'..k,true,
	dreem..'mute_forward'..k,true,
	dreem..'mute_contact'..k,true,
	dreem..'mute_document'..k,true,
	dreem..'mute_inline'..k,true,
	dreem..'lock_username'..k,true,
	dreem..'replay'..k,true
	)
	redis:sadd(dreem..'group:ids',k) 

	if IDS.Admins then
	for user,ID in pairs(IDS.Admins) do
	redis:hset(dreem..'username:'..ID,'username',user)
	redis:sadd(dreem..'admins:'..k,ID)
	end
	end
	if IDS.Creator then
	for user,ID in pairs(IDS.Creator) do
	redis:hset(dreem..'username:'..ID,'username',user)
	redis:sadd(dreem..':MONSHA_BOT:'..k,ID)
	end
	end
	if IDS.Owner then
	for user,ID in pairs(IDS.Owner) do
	redis:hset(dreem..'username:'..ID,'username',user)
	redis:sadd(dreem..'owners:'..k,ID)
	end
	end
	end
	io.popen("rm -fr ../.telegram-cli/data/document/*")
	sendMsg(Uploaded_Groups_CH,Uploaded_Groups_MS,'📦*¦* تم رفع آلنسـخهہ‏‏ آلآحتيآطـيهہ\n⚖️*¦* حآليآ عدد مـجمـوعآتگ هہ‏‏يهہ‏‏ *'..redis:scard(dreem..'group:ids')..'* 🌿\n✓')
	end
	elseif data.ID == "UpdateUser" then  
	if data.user_.type_.ID == "UserTypeDeleted" then
	print("¦ userTypeDeleted")
	redis:srem(dreem..'users',data.user_.id_)
	elseif data.user_.type_.ID == "UserTypeGeneral" then
	local CheckUser = redis:hgetall(dreem..'username:'..data.user_.id_).username
	...
	redis:hset(dreem..'username:'..data.user_.id_,'username',USERNAME)
	end 
	end
